...
For this project, I used the `develop` branch, of the **IOHK** repository, as my feature branch. The work has not been merged into the `main` (or `master`) branch due to the exploratory nature of this analysis. The decision to use a Jupyter Notebook (`.ipynb`) instead of a Python script was also driven by the need for flexibility and iteration during the exploratory phase. 

Had this project been intended for production, the workflow would have followed a more structured approach:
1. Create a dedicated feature branch for the analysis.
2. Ensure the code met production standards.
3. Conduct a peer-review process with at least two reviewers.
4. Merge the feature branch into the `develop` branch only after addressing all feedback.

The current implementation does not follow OOP principles explicitly. It is structured procedurally, focusing on standalone functions and step-by-step operations. I found this acceptable due to the scale of the project.
...
- **Currency Consistency**:  
  While it is understood that payment currency should align with the loan currency, this validation has been excluded from the analysis for the purpose of this assignment.
...
To benchmark SwiftLoan against CashConnect and provide actionable insights, the following was done:

1. **Currency Conversion for Loan Book Size:**
   - SwiftLoan operates across Kenya and Uganda, requiring consistent comparison in USD for clarity and relevance to stakeholders.
   - I utilized current exchange rates to maintain accuracy and simplicity since historical rates were unavailable.

2. **Loan Book Size and Distribution:**
   - Calculated the total loan book value to assess scale and growth.
   - Analyzed portfolio composition (UGX vs. KES) to identify regional financial dynamics.

3. **Exploratory Analysis of Key Metrics:**
   - Summarized loan sizes and terms to highlight operational similarities with CashConnect.
   - Investigated trends in loan issuance over time to capture growth trajectory and lending demand.

4. **Demographic and Geographic Breakdown:**
   - Evaluated borrower demographics (gender, country) to understand customer profiles and inform targeted strategies.
   - Analyzed arrears patterns to identify regional performance differences and opportunities for intervention.

5. **Product Performance Analysis:**
   - Examined arrears and repayment performance by product type to identify potential issues in product design or borrower risk assessment.

6. **Visualization and Communication:**
   - Used graphs and charts to simplify complex data, enhancing interpretability for stakeholders.
